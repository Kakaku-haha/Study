...
def notExisted(url,i):
    for row in range(1,i):
        if url == ws.cell(row,4).value :
            print("已存在！")
            return False
    return True    
...
for i in range(1,len(ws['A'])):  
    ...
    try:
        if url != None and notExisted(url,i):
            response = requests.get(url,headers=headers)
            content = response.content
            username = str(ws.cell(i,1).value).replace('c\n','')
            with open('C:/Users/ZH/Downloads/vide/{}.mp4'.format(username),'wb') as f:
                f.write(content)
                print("下载成功")
    except:print("nooo!")